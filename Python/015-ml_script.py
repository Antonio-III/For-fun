...
def data_preprocessing(df: pd.DataFrame, dependent_var: str, str_in_data: bool, str_columns_dum: list, str_columns_cvector: list) -> tuple[pd.DataFrame, pd.Series]:
    """
    Step 2 of the Machine Learning steps. Converts textual columns to numerical data. Returns 2 objects: The independent variable (X) and the dependent variable (y).
    """
    if str_in_data:
...
def convert_text_col_to_num_cvector(df: pd.DataFrame, str_columns: list) -> pd.DataFrame:
    ...
    for column in str_columns:
        ...
        transformed = vectorizer.fit_transform(df[column].fillna(""))
...
def data_splitting(X: pd.DataFrame, y: pd.Series, test_size: float, random_state: int) -> tuple[pd.DataFrame, pd.Series | pd.DataFrame, pd.DataFrame, pd.Series | pd.DataFrame]:
    """
...
def set_random_state(seed: str) -> int:
    ...
    try:
        ...
        random_state = seed if seed >= MIN_SEED and seed <= MAX_SEED else generate_random_seed(ommit_error=True)
    finally:
        return random_state 
